package com.example.myandroiddemo.notification;


import java.io.File;

import com.example.myandroiddemo.R;
import com.example.myandroiddemo.notification.BaseActivity;
 ...
 * 通知栏应用
 * 
 * 设置提醒标志符Flags，功能：提醒标志符，向通知添加声音、闪灯和振动效果等设置达到通知提醒效果，可以组合多个属性
有2种设置方法：
 ...
 */
public class MyNotificationDemoActivity extends BaseActivity implements OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.nitification);
		initView();
		initNotify();
	}
	...
	private void initNotify(){
		...
		mBuilder.setContentTitle("MyAndroidDemo")
				.setContentText("MyAndroidDemo发出了一个通知")
				.setContentIntent(getDefalutIntent(Notification.FLAG_AUTO_CANCEL))//设置通知栏点击意图，有好几种点击意图
...
//				.setAutoCancel(true)//设置这个标志当用户单击面板就可以让通知将自动取消  
				...
				.setDefaults(Notification.DEFAULT_SOUND|Notification.DEFAULT_LIGHTS)//向通知添加声音、闪灯和振动效果的最简单、最一致的方式是使用当前的用户默认设置，使用defaults属性，可以组合：
			//	Notification.DEFAULT_ALL  Notification.DEFAULT_SOUND 添加声音 // requires VIBRATE permission
				.setSmallIcon(R.drawable.ic_launcher);
		//mNotificationManager.notify(notifyId, mBuilder.build());添加这句可以使这条初始化的默认通知生效，
	}
...
//		3.NotificationCompat.InboxStyle  收件箱风格：显示多行文字
		...
		String[] events = {"one","two","three","four","five"};
...
//		Notification.FLAG_FOREGROUND_SERVICE    //表示
		//在通知栏上点击此通知后自动清除此通知，第一种设置flag的方法
...
//		notification.flags = Notification.FLAG_AUTO_CANCEL; //在通知栏上点击此通知后自动清除此通知
		...
		Intent resultIntent = new Intent(this, MyNotificationDemoActivity.class);
